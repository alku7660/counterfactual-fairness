...
class Graph:
    ...
    def get_feat_possible_values(self, data, obj=None, points=None):
        ...
        feat_possible_values_all_centroids = {}
        for c_idx in range(len(normal_centroids)):
            for cluster_instance_idx in self.feature_clusters[c_idx]:
                centroid_cluster_instance = data.transformed_false_undesired_test_df.loc[cluster_instance_idx].values
                cf_feat_possible_values = {}
                for k in range(len(points)):
                    train_cf_k = points[k]
                    v = centroid_cluster_instance - train_cf_k
                    nonzero_index = list(np.nonzero(v)[0])
                    feat_checked = []
                    feat_possible_values = []
                    for i in range(len(centroid_cluster_instance)):
                        if i not in feat_checked:
                            feat_i = data.processed_features[i]
                            if feat_i in data.bin_enc_cols:
                                if i in nonzero_index:
                                    value = [train_cf_k[i], centroid_cluster_instance[i]]
                                else:
                                    value = [train_cf_k[i]]
                                feat_checked.extend([i])
                            elif feat_i in data.cat_enc_cols:
                                ...
                                nn_cat_idx = list(train_cf_k[idx_cat_i])
                                if any(item in idx_cat_i for item in nonzero_index):
                                    ioi_cat_idx = list(centroid_cluster_instance[idx_cat_i])
                                    value = [nn_cat_idx, ioi_cat_idx]
                                else:
                                    value = [nn_cat_idx]
                                feat_checked.extend(idx_cat_i)
                                ...
                                if i in nonzero_index:
                                    values_i = list(data.processed_feat_dist[feat_i].keys())
                                    max_val_i, min_val_i = max(centroid_cluster_instance[i], train_cf_k[i]), min(centroid_cluster_instance[i], train_cf_k[i])
                                    value = [j for j in values_i if j <= max_val_i and j >= min_val_i]
                                else:
                                    value = [train_cf_k[i]]
                                feat_checked.extend([i])
                            ...
                            feat_possible_values.append(value)
                    cf_feat_possible_values[k] = feat_possible_values
                feat_possible_values_all_centroids[cluster_instance_idx] = cf_feat_possible_values
        return feat_possible_values_all_centroids
        ...
        for c_idx in range(len(self.feature_centroids)):
            for cluster_instance_idx in self.feature_clusters[c_idx]:
                for k in range(len(self.train_cf)):
                    print(f'Graph: cluster instance {cluster_instance_idx} and training CF {k}. Nodes size: {len(graph_nodes)}')
                    feat_possible_values_k = feat_possible_values[cluster_instance_idx][k]
                    ...
                    for i in permutations:
                        ...
                        if model.model.predict(perm_i.reshape(1, -1)) != self.ioi_label and \
                            ...
                            not any(np.array_equal(perm_i, x) for x in self.train_cf): #  and \ verify_feasibility(normal_centroid, perm_i, data)
